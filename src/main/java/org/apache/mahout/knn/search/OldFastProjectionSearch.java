...
/**
 * Does approximate nearest neighbor search by projecting the referenceVectors.
 ...
 */
public class OldFastProjectionSearch extends UpdatableSearcher implements Iterable<Vector> {
  /**
   * The actual vectors.
  ...
  public OldFastProjectionSearch(DistanceMeasure distanceMeasure, int numProjections, int searchSize) {
  ...
  public boolean remove(Vector vector, double epsilon) {
    ...
    for (int i = 0; i < projections.size(); i++) {
      ...
      while (r >= 0 && r < referenceVectors.size() && projections.get(i)[r] >= projection -
          epsilon2) {
        r--;
      }
      while (r >= 0 && r < referenceVectors.size() && projections.get(i)[r] <= projection +
    ...
    return Iterators.concat(
        new AbstractIterator<Vector>() {
          Iterator<Vector> data =
              OldFastProjectionSearch.this.referenceVectors.iterator();
