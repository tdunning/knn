...
public abstract class AbstractSearchTest {
  ...
  public void testExactMatch() {
    ...
    List<WeightedThing<WeightedVector>> r = s.search(q, 2);
  ...
  public void testNearMatch() {
    ...
    for (MatrixSlice slice : queries) {
      ...
      List<WeightedThing<WeightedVector>> r = s.search(query, 2);
  ...
  public void testOrdering() {
    ...
    for (MatrixSlice query : queries) {
      List<WeightedThing<WeightedVector>> r = s.search(query.vector(), 200);
      double x = 0;
      for (WeightedThing<WeightedVector> thing : r) {
  ...
  public void testSmallSearch() {
    ...
    for (MatrixSlice row : m) {
      final List<WeightedThing<WeightedVector>> r = s.search(row.vector(), 3);
  ...
  public void testRemoval() {
    ...
    if (s instanceof UpdatableSearcher) {
      ...
      List<WeightedThing<WeightedVector>> r0 = s.search(x.get(0), 2);

      s.remove(x.get(0), 1e-7);
      assertEquals(size0 - 1, s.size());

      List<WeightedThing<WeightedVector>> r = s.search(x.get(0), 1);
  ...
  public List<WeightedVector> subset(Iterable<WeightedVector> data, int n) {
    ...
    for (WeightedVector row : data) {
      if (r.size() < n) {
        ...
        int k = gen.nextInt(row.getIndex());
        if (k < r.size()) {
